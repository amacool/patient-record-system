    ...
    <div class="col-md-9 col-md-offset-3">
        ...
        <div class="panel panel-default">
            <div class="panel-heading">
                ...
                <a class="btn btn-default btn-sm pull-right" href="{{ route('clients.records.printshow', [$client->id, $record->id]) }}" role="button">UTSKRIFTSVERSJON</a>
                @if ($record->created_by == \Auth::user()->id)
                    @if ($record->signed_by !== null)
                        <a class="btn btn-default btn-sm pull-right" href="{{ route('clients.records.unsignform', [$client->id, $record->id]) }}" role="button">AVSIGNER</a>
                    @endif
                        @if ($record->signed_by == null)
                            <a class="btn btn-default btn-sm pull-right" href="{{ route('clients.records.edit', [$client->id, $record->id]) }}" role="button">ENDRE</a>
                        @endif
                @endif
                <a class="btn btn-default btn-sm pull-right" href="{{ route('clients.records.changehistory', [$client->id, $record->id]) }}" role="button">HISTORIE</a>
                <br/>
