...
@section('content')
    ...
    <table class="table">
        ...
        <tr>
            <td>{{$user->name}}</td>
            <td><a class="btn btn-default" href="{{ route('clients.removeaccessform', [$client->id, $user->id]) }}" role="button">FJERN TILGANG</a></td>
        </tr>
        @endforeach
...
@foreach ($restofcompany as $user)
    ...
    @unless ($user->id == \Auth::user()->id)
        <tr>
        <td>{{$user->name}}</td>
        <td><a class="btn btn-default" href="{{ route('clients.accessform', [$client->id, $user->id]) }}" role="button">GI TILGANG</a></td>
        </tr>
    @endunless
@endforeach
